...
export class PQDistance {
  ...
  async load(): Promise<void> {
    ...
    this.codebooksTensor = new ort.Tensor('float32', this.codebooks!, [this.metadata!.m, this.metadata!.k, this.metadata!.d_sub])
    
    console.log(`PQ system loaded: m=${this.metadata!.m}, k=${this.metadata!.k}, d_sub=${this.metadata!.d_sub}`)
  }
  ...
  private async loadCodebooks(): Promise<void> {
    ...
    if (this.codebooks.length !== expectedSize) {
      throw new Error(
        ...
        `(m=${this.metadata!.m}, k=${this.metadata!.k}, d_sub=${this.metadata!.d_sub})`
      )
    }
  }
    ...
    for (let subq = 0; subq < this.metadata.m; subq++) {
      ...
      for (let centroidIdx = 0; centroidIdx < k; centroidIdx++) {
        ...
        for (let dim = 0; dim < this.metadata.d_sub; dim++) {
          const diff = queryResidual[querySubvectorStart + dim] - this.codebooks[codebookOffset + dim]
          dist += diff * diff
        }
