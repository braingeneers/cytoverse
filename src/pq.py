...
class PQDistanceBase(nn.Module):
    ...
    def forward(
        ...
        batch_indices = torch.arange(m, device=reference_codes.device).unsqueeze(0).expand(N, -1)  # [N, m]
...
def train_pq_codebooks(
    ...
    kmeans = KMeansComplete(max_iterations=max_iterations)
    ...
    for i in range(m):
        ...
        centroids, assignments = kmeans(
            subvectors,
            torch.tensor(k),
            torch.tensor(42 + i)  # Different seed for each subspace
        )
...
def export_pq_models(codebooks: torch.Tensor, output_dir: str | Path = ".", k_nn: int = 50):
    """
    ...
    print("- pq_distance_base_optimized.onnx: Optimized distance computation (if successful)")
