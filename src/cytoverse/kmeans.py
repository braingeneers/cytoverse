...
class KMeansIteration(nn.Module):
    ...
    def forward(
        ...
        assignments_one_hot = torch.nn.functional.one_hot(assignments, num_classes=k).float()  # [N, K]
...
class KMeansInit(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, k: torch.Tensor, seed: torch.Tensor) -> torch.Tensor:
        """
...
class KMeansComplete(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.max_iterations):
            centroids, assignments, converged = self.kmeans_iter(
