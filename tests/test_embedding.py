...
class TestEmbeddingData:
    ...
    def test_study_id(self, adata):
        ...
        assert len(study_ids) == 1, f"Expected exactly one study ID, got {len(study_ids)}: {study_ids}"
        
        # The internal study ID for GSE136831 should be 'DS000011735'
        expected_study_id = 'DS000011735'
        actual_study_id = study_ids[0]
        
        assert actual_study_id == expected_study_id, f"Expected study ID '{expected_study_id}', got '{actual_study_id}'"
    ...
    def test_study_in_scimilarity_db(self, adata, metadata_db):
        ...
        assert is_present, f"GSE136831 study '{h5ad_study_id}' should be present in the SCimilarity database"
    ...
    def test_study_in_labels_parquet(self, adata):
        ...
        assert study_present, f"Study '{h5ad_study_id}' should appear at least once in labels.parquet"
    ...
    def test_sample_embedding_validation(self, adata, embeddings_db):
        ...
        parquet_samples = labels_df[labels_df['study'] == h5ad_study_id]['sample'].unique()
        common_samples = set(h5ad_samples) & set(parquet_samples)
        
        assert len(common_samples) > 0, "No common samples found between H5AD and parquet files"
        ...
        assert len(sample_indices) > 0, f"No embedding indices found for sample {target_sample}"
        ...
        assert embedding.dtype == 'float32', f"Expected float32 dtype, got {embedding.dtype}"
        ...
        for i, val in enumerate(first_5_dims):
            assert -10.0 <= val <= 10.0, f"Dimension {i} value {val} is outside reasonable range [-10, 10]"
