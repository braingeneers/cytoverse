...
class TestIVFPQComplete:
    ...
    def test_ivfpq_vs_scimilarity_labels(self):
        ...
        try:
            ...
            pytest.skip(f"SCimilarity dependencies not available: {e}")

        # Load test data (10 cells)
        test_data_path = Path("tests/GSE136831_subsample_10.h5ad")
        ...
        scimilarity_model_path = Path("data/scimilarity/model_v1.1")
        if not scimilarity_model_path.exists():
            pytest.skip(f"SCimilarity model not found: {scimilarity_model_path}")
        ...
        if not ivfpq_model_path.exists():
            pytest.skip(f"IVFPQ model not found: {ivfpq_model_path}")

        # Add counts layer if needed
        if "counts" not in adata.layers:
            adata.layers["counts"] = adata.X.copy()

        # Align and normalize data for SCimilarity
        print("Aligning and normalizing data...")
        adata_aligned = align_dataset(adata, scimilarity_model.gene_order)
        ...
        # Get SCimilarity ground truth predictions using CellSearch
        print("Getting SCimilarity CellSearch predictions...")
        scimilarity_results = scimilarity_search.search(adata_normalized, k=1)
        scimilarity_labels = [result[0]["prediction"] for result in scimilarity_results]
        print(f"SCimilarity predictions: {scimilarity_labels}")

        # Load IVFPQ model and get predictions
        print("Loading IVFPQ model...")
        ivfpq = IVFPQ(ivfpq_model_path)

        # Load SCimilarity labels for IVFPQ lookup
        labels_path = Path("data/scimilarity/labels.parquet")
        if not labels_path.exists():
            pytest.skip(f"SCimilarity labels not found: {labels_path}")

        labels_df = pd.read_parquet(labels_path)
        print(f"Loaded labels for {len(labels_df)} training vectors")

        # Get IVFPQ predictions for all embeddings
        ...
        for i in range(computed_embeddings.shape[0]):
            ...
            if len(vector_ids) > 0 and vector_ids[0] in labels_df.index:
                ...
                ivfpq_labels.append(pred_label)
            else:
                ivfpq_labels.append("Unknown")

        print(f"IVFPQ predictions: {ivfpq_labels}")
        ...
        if valid_comparisons > 0:
            ...
            print(f"\n📊 IVFPQ vs SCimilarity CellSearch Comparison:")
            print(f"   Valid comparisons: {valid_comparisons}/{len(adata)}")
            print(f"   Matches: {matches}")
            print(f"   Match percentage: {match_percentage:.1f}%")

            # The test passes if we can perform the comparison
