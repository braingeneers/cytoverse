...
describe('PQDistance', () => {
  ...
  describe('PQ Encoding with ONNX', () => {
    ...
    it('should throw error if encode model not loaded', async () => {
      ...
      await expect(pqDistance.encode(residualVector)).rejects.toThrow(
        'Encode model not loaded. Call loadEncodeModel() first.'
      );
    });
    ...
    it('should throw error if PQ system not loaded for encoding', async () => {
      ...
      await expect(pqDistance.encode(residualVector)).rejects.toThrow(
        'PQ system not loaded. Call load() first.'
      );
    });
  });
  ...
  describe('CPU Search Fallback', () => {
    it('should compute asymmetric distances on CPU', async () => {
      ...
      global.fetch = vi
        ...
        .mockResolvedValueOnce({
          ok: true,
          arrayBuffer: () => Promise.resolve(codebooks.buffer),
        });
      ...
      const pqCodesUint8 = new Uint8Array([
        0,
        0, // Vector 0: uses centroids [0,0] from both subquantizers
        1,
        1, // Vector 1: uses centroids [1,1]
        0,
        1, // Vector 2: uses centroids [0,1]
      ]);
    ...
    it('should throw error if CPU search called without system loaded', () => {
      ...
      expect(() => pqDistance.searchCPU(queryResidual, pqCodes)).toThrow(
        'PQ system not loaded'
      );
    });
  });
});
