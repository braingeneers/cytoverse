...
test('Navigate to root, run analysis, download and validate CSV', async ({ page }) => {
  ...
  // Parse CSV content
  const records: Array<{ cell_id: string; category_label: string; confidence?: string }> = parse(
    csvContent,
    {
      columns: true,
  ...
  const matchPercentageConf1 =
    totalWithConfidence1 > 0 ? (matchingWithConfidence1 / totalWithConfidence1) * 100 : 0
  const matchPercentageConf09 =
    totalWithConfidence09 > 0 ? (matchingWithConfidence09 / totalWithConfidence09) * 100 : 0

  console.log(
    `Total matching labels: ${matchingCount}/${totalCount} (${matchPercentage.toFixed(2)}%)`
  )
  console.log(
